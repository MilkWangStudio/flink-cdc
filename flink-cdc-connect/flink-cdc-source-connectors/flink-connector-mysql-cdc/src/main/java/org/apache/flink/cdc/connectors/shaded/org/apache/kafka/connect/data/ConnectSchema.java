...
package org.apache.flink.cdc.connectors.shaded.org.apache.kafka.connect.data;
...
public class ConnectSchema implements Schema {
    ...
    static {
        ...
        SCHEMA_TYPE_CLASSES.put(Type.INT64, list);
        ...
        // Bytes are special and have 2 representations. byte[] causes problems because it doesn't
        // handle equals() and
        // hashCode() like we want objects to, so we support both byte[] and ByteBuffer. Using plain
        // byte[] can cause
        ...
        LOGICAL_TYPE_CLASSES.put(
                Date.LOGICAL_NAME, Collections.singletonList(java.util.Date.class));
        LOGICAL_TYPE_CLASSES.put(
                Time.LOGICAL_NAME, Collections.singletonList(java.util.Date.class));
        LOGICAL_TYPE_CLASSES.put(
                Timestamp.LOGICAL_NAME, Collections.singletonList(java.util.Date.class));
        // We don't need to put these into JAVA_CLASS_SCHEMA_TYPES since that's only used to
        // determine schemas for
        // schemaless data and logical types will have ambiguous schemas (e.g. many of them use the
        // same Java class) so
    ...
    // Optional name and version provide a built-in way to indicate what type of data is included.
    // Most
    // useful for structs to indicate the semantics of the struct and map it to some existing
    // underlying
    // serializer-specific schema. However, can also be useful in specifying other logical types
    // (e.g. a set is an array
    ...
    /**
     * Construct a Schema for a primitive type, setting schema parameters, struct fields, and key
     * and value schemas to null.
     */
    public ConnectSchema(
            Type type,
            boolean optional,
    ...
    /**
     * Construct a default schema for a primitive type. The schema is required, has no default
     * value, name, version, or documentation.
     */
    ...
    /**
     * Validate that the value can be used with the schema, i.e. that its type matches the schema
     * type and nullability requirements. Throws a DataException if the value is invalid.
     *
    ...
    public static void validateValue(String name, Schema schema, Object value) {
        ...
        log.trace(
                "ConnectSchema validateValue, name={}, schema={}, value={}, found={}",
                name,
                schema,
                value,
                foundMatch);
    ...
    private static List<Class<?>> expectedClassesFor(Schema schema) {
        ...
        if (expectedClasses == null) {
            ...
            expectedClasses =
                    SCHEMA_TYPE_CLASSES.getOrDefault(schema.type(), Collections.emptyList());
        }
        return expectedClasses;
    }

    /**
     * Validate that the value can be used for this schema, i.e. that its type matches the schema
     * type and optional requirements. Throws a DataException if the value is invalid.
     *
